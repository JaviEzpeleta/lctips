...
export type ThankYouRankingEntry = {
  ...
  total: number
  count: number
  /** Personalized handwritten note, generated by Gemini at export time from
   *  this friend's recent Lens posts. Optional: only the top ~10 get one, and
   *  only when GEMINI_API_KEY is configured — otherwise the video just uses
   *  the cute hardcoded line. */
  note?: string
}
